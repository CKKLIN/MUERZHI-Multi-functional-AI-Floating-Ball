...
electron.contextBridge.exposeInMainWorld("electronAPI", {
	...
	onMainWindowClose: (callback) => {
		const handler = () => callback();
		electron.ipcRenderer.on("app-main-window-close", handler);
		return () => electron.ipcRenderer.removeListener("app-main-window-close", handler);
	},
	...
	getMediaDevices: () => {
		...
		return md.enumerateDevices().then((devices) => devices.map((d) => ({
			deviceId: d.deviceId,
			kind: d.kind,
			label: d.label
		})));
	...
	getAppI18n: () => electron.ipcRenderer.invoke("get-app-i18n"),
	onAppLocaleChanged: (callback) => {
		const handler = (_event, data) => callback(data);
		electron.ipcRenderer.on("app-locale-changed", handler);
		return () => electron.ipcRenderer.removeListener("app-locale-changed", handler);
	},
	...
	onTodoDataChanged: (callback) => {
		...
		electron.ipcRenderer.on("todo-data-changed", handler);
		return () => electron.ipcRenderer.removeListener("todo-data-changed", handler);
	},
