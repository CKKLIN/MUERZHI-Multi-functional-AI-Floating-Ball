...
var import_main, logDir, getDate, logger_default;
var init_logger = __esmMin((() => {
	import_main = /* @__PURE__ */ __toESM(require_main());
	import_main.default.initialize();
	logDir = electron.app.isPackaged ? (0, node_path.join)(electron.app.getPath("userData"), "logs") : (0, node_path.join)(electron.app.getAppPath(), "src", "log");
	getDate = () => {
		const d = /* @__PURE__ */ new Date();
		return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
	};
	import_main.default.transports.file.resolvePathFn = (variables) => {
		return (0, node_path.join)(logDir, getDate() + ".log");
	};
