...
async function collapseBall() {
	...
	try {
		await floatingBallWindow.webContents.executeJavaScript(`var s=document.getElementById('ringSvg');while(s.firstChild){s.removeChild(s.firstChild)} menuCreated=false; void 0;`);
	} catch {}
	if (!floatingBallWindow || floatingBallWindow.isDestroyed()) return;
	floatingBallWindow.setOpacity(0);
	floatingBallWindow.setBounds({
